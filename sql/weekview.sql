CREATE OR REPLACE VIEW week_chart_view AS
SELECT 
...
FROM (
  ...
  TO_CHAR(TRUNC(TO_CHAR(be_date, 'HH24')/6+1)*6, 'FM00') || ':00' AS label,
...
WHERE TRUNC(be_date) BETWEEN TRUNC(SYSDATE-7, 'D') AND TRUNC(SYSDATE, 'D') - 1
    AND b_id = 'gta'
GROUP BY TO_CHAR(TRUNC(TO_CHAR(be_date, 'HH24')/6+1)*6, 'FM00') || ':00', TO_CHAR(be_date, 'DY'), TO_CHAR(be_date, 'YYYY-MM-DD') || ' ' || TO_CHAR(TRUNC(TO_CHAR(be_date, 'HH24')/6+1)*6, 'FM00') || ':00'
ORDER BY day
) yesterday
FULL OUTER JOIN (
    SELECT 
...
FULL OUTER JOIN (
  ...
  TO_CHAR(TRUNC(TO_CHAR(MODEL_DATE, 'HH24')/6+1)*6, 'FM00') || ':00' AS label,
...
WHERE TRUNC(MODEL_DATE) BETWEEN TRUNC(SYSDATE+7, 'D') AND TRUNC(SYSDATE+7, 'D') + 6
    AND division = 'total'
GROUP BY TO_CHAR(TRUNC(TO_CHAR(MODEL_DATE, 'HH24')/6+1)*6, 'FM00') || ':00', TO_CHAR(MODEL_DATE, 'DY'), TO_CHAR(MODEL_DATE, 'YYYY-MM-DD') || ' ' || TO_CHAR(TRUNC(TO_CHAR(MODEL_DATE, 'HH24')/6+1)*6, 'FM00') || ':00'
ORDER BY day
